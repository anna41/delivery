...
import buildDirectionForCar from '../helpers/directionBuilderForCar';
import buildDirectionForOrder from '../helpers/directionBuilderForOrder';


const MapWithADirectionsRenderer = compose(
  withProps({
    googleMapURL: "https://maps.googleapis.com/maps/api/js?key=AIzaSyBUjFynG_oDGBJ05AVwTq_etAIqAvEd-rM&libraries=geometry,drawing,places",
    loadingElement: <div style={{ height: `90%` }} />,
    containerElement: <div style={{ height: `90%`, padding: '2%' }} />,
  ...
  <GoogleMap
    defaultZoom={8}
    defaultCenter={props ?
      new google.maps.LatLng(props[0].lat, props[0].lng)
      : new google.maps.LatLng(49.84075020419229, 24.030532836914062)}
  ...
  >
    {props.directions && props.directions.request.waypoints.length != 0 && <DirectionsRenderer directions={props.directions} />}
    {console.log("prrops",props)}
    {props.directions
      && props.directions.request.waypoints.length == 0 && <Marker
        position={new google.maps.LatLng(props[0].lat, props[0].lng)} >
      </Marker>}
  </GoogleMap>
